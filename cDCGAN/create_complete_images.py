...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_score', type=float, default=2.948633233877742, help="Don't save latents and images for scores below.")
    ...
    if args.find_best_num_stars_per_image:
        for i in range(num_rounds):
            ...
            # todo: when cDCGAN class has been rewritten, use detransform method
        ...
        print("Score RF: {} +- {}. Score NN; {} +- {}".format(np.mean(rf_score), np.std(rf_score), np.mean(nn_score),
                                                              np.std(nn_score)))
