...
# parames for model 1
def get_model():
    kernel = (conf['kernel'], conf['kernel'])
    ...
    while res > min_res:
        if res % 2 != 0:
            ...
            model.add(Padder(padding=closest2res - res,
                             input_shape=(res, res, image_channels)))  # 125 -> 128 or maybe 25 -> 32...
            res = closest2res
        for i in range(conf['num_convs_per_downsample']):
            strides = 2 if i == 0 else 1
            model.add(tf.keras.layers.Conv2D(features, kernel, padding='same', strides=strides, use_bias=conf['use_bias'],
                                             input_shape=(res, res, image_channels)))  # depth wrong for following convs
