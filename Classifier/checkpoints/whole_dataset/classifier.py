...
model_kind = conf['model_kind']  # Till or Hannes
...
def load_dataset():
  global x, y, test_images, test_labels
  # often got a memory error in this method, probably due to the previous dataset is not removed soon enough
  #gc.collect()
  x = np.memmap(os.path.join(cp_dir_time, 'trainset.dat'), dtype=np.float32, mode='w+',
                shape=(int(dataset_len*percentage_train), image_size, image_size, 1))
  if percentage_train < 1:
    test_images = np.memmap(os.path.join(cp_dir_time, 'testset.dat'), dtype=np.float32, mode='w+',
  ...
  if not use_dummy_dataset:
    ...
    #print("Test suffled indices: {}".format(indices))
    labels = []
    imgs = []
    for num, ind in enumerate(indices):
      labels.append(label_list[ind][1])
    ...
    print("")
    y = np.stack(labels[:train_len])
    if percentage_train < 1:
      test_labels = np.stack(labels[train_len:])
      print("Shape x: {}\nShape y: {}\nShape test_imgs: {}\nShape test labels: {}"
            .format(x.shape, y.shape, test_images.shape, test_labels.shape))

  else:
    y = np.ones((train_len, 1))
    test_labels = np.ones((test_len, 1))
    x = np.random.normal(0, 1, (train_len, image_size, image_size, image_channels))
...
# parames for model 
class Hannes_model(keras.Model):
  def __init__(self):
    super(Hannes_model, self).__init__()
    kernel = (5, 5)
    features = 32
    max_features = 256
    num_layers=4
    ...
    self.model = tf.keras.Sequential(name='hannes_model')

    for i in range(4):
      self.model.add(layers.Conv2D(
          features, kernel, strides=(2, 2), padding='same', use_bias=use_bias,
          kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.0002, seed=None)))
      if i != 0:
        self.model.add(getNormLayer(norm_type))
      self.model.add(layers.LeakyReLU(alpha=0.2))

    self.model.add(layers.Flatten())
    self.model.add(layers.Dense(1))  # no sigmoid here, so no need to normalize labels
...
checkpoint_dir = os.path.join(home_dir, "Classifier/checkpoints/{}".format(model_kind))
...
if test_on_query:
  ...
  epochs = conf['epochs_per_data_load']
  ...
  if not restore_checkpoint:
    ...
    cp_command = 'cp {} {}'.format(os.path.join(home_dir, "Classifier/{}"), cp_dir_time)
