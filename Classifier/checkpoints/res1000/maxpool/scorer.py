...
with open("scorer_config.yaml", 'r') as stream:
...
if test_on_query:
    with open(os.path.join(cp_dir_time, "scorer_config.yaml"), 'r') as stream:
        conf = yaml.full_load(stream)
    image_size = conf['image_size']
    ...
    if not restore_checkpoint:
        ...
        os.system(cp_command.format("scorer.py"))
        os.system(cp_command.format("scorer_config.yaml"))
