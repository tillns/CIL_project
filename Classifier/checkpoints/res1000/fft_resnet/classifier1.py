        ...
        if self.conf['residual']:
            if self.projection:
                if self.downsample and not self.conf['use_max_pool']:
                    input_tensor = tf.keras.layers.Conv2D(self.features, (self.conf['downsample_factor'],
                                                                          self.conf['downsample_factor']),
                                                          padding='same', strides=self.conf['downsample_factor'],
                                                          use_bias=self.conf['use_bias'])(input_tensor)
                elif conf['use_max_pool']:
                    input_tensor = tf.keras.layers.Conv2D(self.features, (1, 1), padding='same', strides=1,
                                                          use_bias=self.conf['use_bias'])(input_tensor)
                    if self.downsample:
                        input_tensor = tf.keras.layers.MaxPool2D(pool_size=self.conf['downsample_factor'])
            x += input_tensor
        return x
...
if test_on_query:
    with open(os.path.join(cp_dir_time, "config1.yaml"), 'r') as stream:
        conf = yaml.full_load(stream)
    image_size = conf['image_size']
    ...
    if not restore_checkpoint:
        ...
        os.system(cp_command.format("classifier1.py"))
        os.system(cp_command.format("config1.yaml"))
