...
with open("config3.yaml", 'r') as stream:
...
def get_model():
    ...
    for i in range(1, conf['num_dense_layers']):
        model.add(tf.keras.layers.Dense(conf['latent_features'], use_bias=conf['use_bias'], kernel_regularizer=kernel_regularizer))
        model.add(getNormLayer(conf['norm_type']))
        model.add(tf.keras.layers.LeakyReLU(alpha=conf['lrelu_alpha']))
        if conf['dropout'] > 0:
            model.add(tf.keras.layers.Dropout(conf['dropout']))
...
if test_on_query:
    with open(os.path.join(cp_dir_time, "config3.yaml"), 'r') as stream:
        conf = yaml.full_load(stream)
    image_size = conf['image_size']
    ...
    if not restore_checkpoint:
        ...
        os.system(cp_command.format("classifier3.py"))
        os.system(cp_command.format("config3.yaml"))
